...
LINK_SET_EV_DISPATCHER = 'link_set'
...
class Link(object):
    def __init__(self, src, dst):
        ...
        # src and dst are DPPort objects
        self.src = src
        self.dst = dst
...
class LinkSet(dict, app_manager.RyuApp):
    """
    dict: Link -> timestamp
    """
    def __init__(self):
    ...
    def update_link(self, src_dpid, src_port_no, dst_dpid, dst_port_no):
        ...
        if link not in self:
            ...
            self.send_event_to_observers(LinkEvent(link, True))
        self[link] = time.time()
        self._map[src] = dst
    ...
    def link_down(self, link):
        ...
        self.send_event_to_observers(LinkEvent(link, False))
    ...
    def port_deleted(self, dpid, port_no):
        ...
        if rev_link_dst is not None:
            ...
            self.send_event_to_observers(LinkEvent(rev_link, False))
